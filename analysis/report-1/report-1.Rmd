...
1. The current report covers `r nrow(ds)` sampling points, with `r dplyr::n_distinct(ds$substrate)` unique values for `substrate` and `r dplyr::n_distinct(ds$can)` cans.
...
```{r spaghetti, echo=echo_chunks, message=message_chunks, fig.width=7}
```
