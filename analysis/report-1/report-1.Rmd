...
1. Maya's current report covers `r nrow(ds_maya)` sampling points, with `r dplyr::n_distinct(ds_maya$substrate)` unique values for `substrate` and `r dplyr::n_distinct(ds_maya$can)` cans.

1. Valencia's current report covers `r nrow(ds_valencia)` sampling points, with `r dplyr::n_distinct(ds_valencia$substrate)` unique values for `substrate` and `r dplyr::n_distinct(ds_valencia$can)` cans.
...
---------------------------------------------------------------------------
The first plot contains `r dplyr::n_distinct(ds_maya$can)` faint spaghetti strands, each representing a single can's longitudinal trajectory.  Each substrate is assigned a distinct color.  Each substrate has three independent cans, indicated by the plotted digits 1, 2, 3.  Each substrate is summarized by a thick [loess](https://en.wikipedia.org/wiki/Local_regression) curve to capture any nonlinear trend.

The second plot is faceted to better inspect patterns within substrate.

```{r spaghetti-maya, echo=echo_chunks, message=message_chunks, fig.width=7}
```

Scatter Plots - Valencia
---------------------------------------------------------------------------
The first plot contains `r dplyr::n_distinct(ds_valencia$can)` one [regression](https://en.wikipedia.org/wiki/General_linear_model) per substrate, each summarizing the substrate's linear relationship between temperature and pH.  Each substrate is assigned a distinct color and has six independent cans, indicated by the plotted digits 1, 2, ..., 6.

The second plot is faceted to better inspect patterns within substrate.

```{r scatter-valencia, echo=echo_chunks, message=message_chunks, fig.width=7, fig.height = 8}
```
